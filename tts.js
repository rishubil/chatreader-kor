...
/**
 * 이모티콘 TTS 설정
 * @param {String} command d설정 명령어
 * @param {Function} callback 실행 결과 콜백
 */
 function setEmote(command, callback) {
    if (command !== "") {
        let pair = command.split(" ");
		let emote = pair.shift();
        if (pair.length === 0) {
            delete window.emotelist[emote];
            localStorage.setItem("tts_emote_" + window.channelname, JSON.stringify(window.emotelist));
            callback(true, "이모티콘 " + emote + " 를 읽지 않습니다.");
        } else if (pair.length >= 1) {
            let replacer = pair.join(" ");
            window.emotelist[emote] = replacer;
            localStorage.setItem("tts_emote_" + window.channelname, JSON.stringify(window.emotelist));
            callback(true, "이모티콘 " + emote + " 를 " + replacer + " 로 읽습니다.");
        }
    }
}
...
function parseCmd(e) {
    ...
    if (command !== "") {
        ...
        if (command.indexOf("emote ") !== -1) {
            ...
            setEmote(command, displayResultFromChat);
        }
    }
}
...
function parseChat(e) {
    ...
    if (window.initok) {
        ...
        // 트위치 이모티콘 변환
        message = replaceTwitchEmoticon(message, e.emotes, e);
...
function getKoreanEmoticon(data) {
    const parsed_emotes = [];
    if (data.regex == null) {
        return parsed_emotes;
    }
    for (const emote_name in data.regex) {
        if (data.regex.hasOwnProperty(emote_name)) {
            const emote = data.regex[emote_name];
            if (emote.alias.length > 0) {
                parsed_emotes[emote_name] = emote.alias[0].alias;
            }
        }
    }
    return parsed_emotes;
 ...
 * 트위치 이모티콘을 변환
 * 
 * 변환할 모든 이모티콘이 window.emotelist에 존재할 경우에는 해당 값을 이용하여 즉시 변환을 수행
 * window.emotelist에 존재하지 않는 이모티콘은 twiticon에서 조회 요청을 하되,
 * 조회 결과가 비동기적으로 처리되므로 현재 message 전체를 빈 문자열로 반환하고,
 * 조회 완료시 처음부터 다시 처리함
 * 
 * @returns {String} replaced message
 * @param message {String} message to replace
 * @param emotes {String} emote list to replace
 * @param {Object} e 채팅 오브젝트
 */
function replaceTwitchEmoticon(message, emotes, e) {
    ...
    if (typeof emotes != 'undefined') {
        ...
        for (const replace_id in replace_list) {
            if (typeof window.emotelist[replace_id] === 'undefined') {
                $.post(
                    "https://twiticon.com/extension/getemote.php",
                    {
                        data: "[" + $.map(replace_list, function (v, k) { return v }).join(',') + "]",
                    }
                ).done(function (data) {
                    // this is error!
                    for (const replace_id in replace_list) {
                        if (replace_list.hasOwnProperty(replace_id)) {
                            window.emotelist[replace_id] = '';
                        }
                    }
                    localStorage.setItem("tts_emote_" + window.channelname, JSON.stringify(window.emotelist));
                })
                .fail(function (jqXHR) {
                    // this is suceess!
                    const parsed_emotes = getKoreanEmoticon(jQuery.parseJSON(jqXHR.responseText));
                    for (const replace_id in replace_list) {
                        if (parsed_emotes.hasOwnProperty(replace_id)) {
                            window.emotelist[replace_id] = parsed_emotes[replace_id];
                        } else if (replace_list.hasOwnProperty(replace_id)) {
                            window.emotelist[replace_id] = '';
                        }
                    }
                    localStorage.setItem("tts_emote_" + window.channelname, JSON.stringify(window.emotelist));
